...
func newACPClient(ctx context.Context, grokPath string, extraEnv []string, cwd string) (*acpClient, error) {
	grokPath = strings.TrimSpace(grokPath)
	if grokPath == "" {
		grokPath = "grok"
	}
	cmd := exec.CommandContext(ctx, grokPath, "--no-auto-update", "agent", "stdio")
	if cwd != "" {
		cmd.Dir = cwd
	}
	cmd.Env = append(os.Environ(), normalizeGrokEnv(extraEnv)...)
...
func normalizeGrokEnv(extraEnv []string) []string {
	out := append([]string(nil), extraEnv...)
	if envValue("XAI_API_KEY", out) == "" {
		if key := envValue("GROK_CODE_XAI_API_KEY", out); key != "" {
			out = append(out, "XAI_API_KEY="+key)
		}
	}
	return out
}

func envValue(name string, extraEnv []string) string {
	prefix := name + "="
	for i := len(extraEnv) - 1; i >= 0; i-- {
		if strings.HasPrefix(extraEnv[i], prefix) {
			return strings.TrimSpace(strings.TrimPrefix(extraEnv[i], prefix))
		}
	}
...
func handshake(ctx context.Context, c *acpClient, cwd string, mcpServers []any) (string, error) {
	initResult, err := c.call(ctx, "initialize", map[string]any{
		...
		"clientInfo": map[string]string{"name": "pangaea-grok-build-acp", "version": "0.1"},
	})
	if err != nil {
		return "", fmt.Errorf("initialize: %w", err)
	}
	methodID, err := selectAuthMethod(initResult)
	if err != nil {
		return "", err
	}
	if methodID != "" {
		if _, err := c.call(ctx, "authenticate", map[string]any{
			"methodId": methodID,
...
func selectAuthMethod(initResult json.RawMessage) (string, error) {
	var parsed struct {
		AuthMethods []struct {
			ID string `json:"id"`
		} `json:"authMethods"`
	}
	_ = json.Unmarshal(initResult, &parsed)
	...
	if override := strings.TrimSpace(os.Getenv("PANGAEA_GROK_ACP_AUTH_METHOD")); override != "" {
		if available[override] || len(available) == 0 {
			return override, nil
		}
		return "", fmt.Errorf("authenticate: configured Grok ACP auth method %q is not available", override)
	}
	if (strings.TrimSpace(os.Getenv("XAI_API_KEY")) != "" || strings.TrimSpace(os.Getenv("GROK_CODE_XAI_API_KEY")) != "") && available["xai.api_key"] {
		return "xai.api_key", nil
	}
	if available["cached_token"] {
		return "cached_token", nil
	}
	if len(available) == 0 {
		return "", nil
	}
	return "", errors.New("authenticate: run `grok login` first or set XAI_API_KEY")
}
...
func sessionPrompt(ctx context.Context, c *acpClient, sessionID, userText string, req compat.Request, stream bool, emit func(compat.Event) error) (stopReason string, reply string, err error) {
	...
	finalText = waitForStableText(ctx, currentText, 2*time.Second, 150*time.Millisecond)
...
func waitForStableText(ctx context.Context, current func() string, maxWait time.Duration, interval time.Duration) string {
	if interval <= 0 {
	...
	defer ticker.Stop()
	last := current()
	seenText := strings.TrimSpace(last) != ""
	stableChecks := 0
	for stableChecks < 2 {
		select {
		case <-ctx.Done():
